...
namespace HotelManager.DAO
{
    public class BillDAO
    {
        ...
        internal DataRow ShowBillRoom(int idRoom)
        {
            ...
            DataTable result = DataProvider.Instance.ExecuteQuery(query, new object[] { DateTime.Now.Date, idRoom });

            if(result.Rows.Count == 0)
            {
                return null; // Return null or handle appropriately if no rows are returned
            }
