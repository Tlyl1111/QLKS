...
namespace HotelManager
{
    public partial class fRoom : Form
    {
        ...
        {
            int roomId;
            if (int.TryParse(comboboxID.Text, out roomId))
            {
                DialogResult result = MessageBox.Show("Bạn có muốn xóa phòng này không?", "Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (result == DialogResult.OK)
                {
                    try
                    {
                        bool deleteSuccess = RoomDAO.Instance.DeleteRoom(roomId);
                        ...
                        {
                            MessageBox.Show("Không thể xóa phòng đang được sử dụng hoặc phòng không tồn tại", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    catch (Exception ex)
                    {
            ...
            {
                MessageBox.Show("Vui lòng chọn phòng khác để xóa", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
