    ...
    public partial class fAddRoom : Form
    {
        ...
        private void InsertRoom()
        {
            ...
            try
            {
                ...
                if (RoomDAO.Instance.CheckRoomNameExists(roomNow.Name, roomNow.IdRoomType))
                {
                    MessageBox.Show("Phòng này đã tồn tại trong loại phòng này. Vui lòng chọn tên khác.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                {
                    MessageBox.Show("Lỗi không thêm được phòng này", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                }
            }
            catch (Exception ex)
            {
